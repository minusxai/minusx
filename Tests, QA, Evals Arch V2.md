...
- [ ] Fixtures: tutorial reset (`beforeAll`/`beforeEach`), faux-LLM reset, logged-in test admin.
- [ ] `flows/e2e.ts` mirroring `flows/node.ts` name-for-name (dispatch→click, `getState`→`window.__MX_STORE__`).
- [ ] Port the jsdom `*.ui.test.tsx` component tests to Playwright; **remove the `ui` Vitest project**.
- **Exit:** local + CI Playwright e2e green, deterministic, offline (faux LLM, tutorial data); `ui` project gone.
