...
> **Status:** Phases 0–5 ✅ landed/CI-green. Phases 0–4 merged (PR #437); the store-expose fix merged (#439);
> QA tooling in PR #440; deploys `qa.yml` on `add-qa-workflow`. Phase 5's runtime gate is **verified against
> live `stg-starlight`**. Remaining: deploy the fix to `some-corp-2`, add `MINUSX_REPO_TOKEN` to the deploys
> repo + merge its `qa.yml`, and **Phase 6 (evals)** — the only phase not started (needs an evals rubric;
> real-LLM, LLM-judged, N-run — kept deliberately separate from CI).
...
### Phase 5 — Prod QA · *depends on: Phase 4* ✅
- [x] **Runtime opt-in instead of a build flag.** Prod builds don't ship `NEXT_PUBLIC_E2E`, so QA exposes
      `window.__MX_STORE__` on a prod build via `?e2e=<E2E_RUNTIME_SECRET>` — middleware validates it, sets
      a signed-ish cookie via a **middleware-issued redirect** (so it survives the onboarding `/`→`/hello-world`
      redirect + reloads), SSR stamps `e2eEnabled`. Hygiene gate only (own-session state). Set `E2E_RUNTIME_SECRET`
      on each deployment to enable; unset ⇒ fully off.
- [x] QA Playwright project (`playwright.qa.config.ts` + `test/qa/*`): targets `QA_BASE_URL` (a deployment) or
      boots a local prod-ish server; `runtime-gate` + `app-smoke` specs. Real LLM is **not** exercised — DOM/Redux
      outcome assertions only (no faux channel on prod), so runs are cheap + side-effect-free.
- [x] `minusxai/deploys` `qa.yml`: `workflow_dispatch` picks a deployment → GitHub **Environment**
      (`sqe-staging`/`sqe-prod`/`minusx-prod`) supplies `QA_BASE_URL` (var) + `QA_EMAIL`/`QA_PASSWORD`/
      `QA_E2E_SECRET` (secrets) → checks out the app repo + runs `npm run test:qa`. Needs `MINUSX_REPO_TOKEN`.
- **Exit:** ✅ `npm run test:qa` green against **live `stg-starlight`** (gate exposes-and-persists).
- **Root-cause learning (the store-expose bug, #439):** the code was never wrong — reproduced the exact
  **turbopack standalone** build with a runtime-only secret and both `process.env` *and* the `@/lib/config`
  constant read it correctly in the middleware bundle. The deployment failure was a **trailing newline/space**
  on `E2E_RUNTIME_SECRET` breaking the length-guarded compare → fixed by `.trim()`ing the configured secret
  (don't reach for `process.env`-with-`eslint-disable` on a hunch; reproduce the real build first).
