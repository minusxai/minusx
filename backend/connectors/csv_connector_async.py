"""Async wrapper for CSV connector (delegates to async DuckDB)"""
...
class AsyncCsvConnector(AsyncDatabaseConnector):
    """
    CSV file-based async connector that wraps AsyncDuckDBConnector.
    ...
    async def test_connection(self) -> dict:
        """Test if the CSV connection is valid (DuckDB database exists and is accessible)."""
        generated_db_path = self.config.get('generated_db_path', '')

        if not generated_db_path:
            return {
                "success": False,
                "message": "No database generated. Please upload CSV files first."
            }
    ...
    def validate_config(self) -> dict:
        """
        Validate CSV connection configuration.

        For CSV connections, we mainly validate that:
        - generated_db_path is present (after CSV upload)
