...
def test_existing_limit_under_max():
    ...
    result = enforce_query_limit(sql, max_limit=10000, dialect='postgres')
    assert "LIMIT 500" in result


def test_existing_limit_over_max():
    sql = "SELECT * FROM users LIMIT 50000"
    result = enforce_query_limit(sql, max_limit=10000, dialect='bigquery')
...
def test_limit_with_offset():
    ...
    result = enforce_query_limit(sql, max_limit=10000, dialect='duckdb')
...
# ---------------------------------------------------------------------------
# Named-parameter preservation across dialects
# ---------------------------------------------------------------------------
