...
- [x] Canvas renderer deleted: `lib/canvas-story/`, `CanvasStoryView`, "Use Canvas Renderer" setting + `useCanvasRenderer` config, Takumi WASM assets, tests; grep shows zero references to `canvas-story`/`takumi`/`useCanvasRenderer`. `dom` surface removed as an option; svg is the only render path. (Also deleted: the `storyRenderer` config union + `resolveStoryRenderer` + Settings toggle + org-config overlay in `lib/data/configs.server.ts`, the share-page `?use=` renderer param, `@takumi-rs/*`/`takumi-js` deps, `public/takumi/`, and the wasm middleware carve-out — ttf/woff2 kept for `public/fonts`. `dom` remains as the story-surface abstraction's second implementation, not selectable anywhere.)
- [x] Capture pipeline: `XMLSerializer` → percent-encoded `data:` URL → `<img>` → canvas → JPEG; awaits `document.fonts.ready` + image decode.
- [x] Styles inside the story root as `data-mx-*` nodes; save paths strip them; byte-identity test (render → save → `content.story` unchanged). (AgentHtml prepends `data-mx-app-styles`/`data-mx-tw`/`data-mx-floating`/`data-mx-fonts` inside the surface root — nothing style-bearing in the iframe `<head>`; `INJECTED_STYLE_SELECTOR` in `lib/html/serialize-story.ts` strips the whole family; byte-identity tests for legacy + jsx in `components/views/shared/__tests__/story-styles-in-root.ui.test.tsx`.)
- [x] Font-asset mechanism with the neutral default's fonts (Phase 3 extends per theme); image inlining. (`lib/data/story/story-fonts.ts`: theme registry → `getStoryFontCss` @font-face CSS over `public/fonts` static assets, injected in-root as `data-mx-fonts` for jsx stories; `serializeStorySvg` splices the data-URI form — fonts and `<img>` srcs — into the parsed copy only.)
- [x] Parsed-copy fixup pass: scroll-offset transforms + `input`/`textarea` value stamping; scroll offsets added to app state; Vega forced to SVG renderer in captures. (Vega is now unconditionally `renderer: 'svg'` — the canvas branch died with the canvas renderer.)
...
- [x] Fluid-width guard green on Chromium + WebKit + Firefox: at container widths 390/768/1104/1440 (text-heavy + wide-table/wide-embed stories) the story root lays out at exactly the container width with no hidden overflow, the serialized capture is exactly the width the reader sees (no fidelity fork), the capture covers the height the content reflowed to, and all of it still holds after the container narrows post-first-paint (side-chat toggle). (Same command — `npm run capture-matrix` → `scripts/story-width-matrix.ts`, run from `capture-matrix.ts`; drives the real `mountStorySurface`/`autoSizeStorySurface`/`STORY_FLUID_SHIM_CSS`/`serializeStorySvg` in a fixture that mirrors AgentHtml's iframe build. jsdom cannot hold this line — it has no layout engine, so `story-fluid-width.ui.test.tsx` only pins which width reaches the surface, in which order.)
