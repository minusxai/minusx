...
- [x] **Move/rename under RLS too**: `updateMetadata`/`moveFolderAndChildren` take the access context — the update policy gates the moved rows (USING) and their target paths (WITH CHECK), folder moves are all-or-nothing (partial policy filtering rolls back). This CLOSED a pre-existing gap: the metadata-only PUT (`moveFile`) had no per-user ownership check at all — any authenticated user could move/rename any file by id. Now scoped to role + home + group grants (admins unaffected). Red-first tests in the same battery (38 total).

**What stays app-side (deliberate):** static type blocklists / protected paths / location restrictions (not per-principal; and cascade deletes must remove `context` children); precise error messages. **Write paths NOT RLS-wrapped, and why:** `DocumentDB.batchSave` is only reachable as the always-rolled-back dry-run pre-flight (real batch saves go through `saveFile`, which IS wired); the share/preview `updateMeta` writes are admin-gated bookkeeping (and preview generation must work for viewers); `appendJsonArray`/`updatePath` have no callers.
