...
**Deliberately NOT deleted, after investigation**: `frontend/lib/chat-v2/` (the `?v=1` legacy-chat toggle) looks unreachable in the current data flow (every write-site hardcodes `version: 3`) but is a legitimate compatibility feature — the v3 backfill migration is a manual, admin-triggered endpoint, not automatic, so a self-hosted deployment that hasn't run it could still have real v1-format conversations. Same reasoning pattern as the M6 "looks abandoned but is load-bearing" findings.
