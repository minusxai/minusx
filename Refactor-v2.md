...
## Post-#568 addendum (2026-07-06) — adversarial completeness audit, second cleanup round

After the "Next PR" section below shipped, the owner asked directly whether all v2 chat code and dead code was really gone. Rather than re-asserting the claim, ran an adversarial audit (3 independent finders + 2 refutation passes trying to disprove the findings) against the actual repo state. Result: the v2-chat claim held up fully (confirmed clean, zero remaining trace of the deleted REST/persistence layer — a handful of unrelated "v2"-named things, like `agents/benchmark-analyst/v2/*`, eval/report agent versioning, and `ConnectionFormV2`/`QuestionViewV2`, are legitimately different axes, not remnants). The dead-code claim did not fully hold up: the audit found real additional dead code, some of it a direct cascade from this PR's own QuestionViewV2 move. Fixed:
- **`FileModal` — an entire unreachable feature**: `openFileModal` (the only action that ever set `modalFile` non-null) had zero dispatch sites, making the whole component (mounted globally in `app/layout.tsx`) permanently dead. Deleted the component + its 4 uiSlice actions/state + selector, as one unit.
- **Redux cascade from the QuestionViewV2 move**: `setSqlEditorCollapsed`/`selectSqlEditorCollapsed` and `addReferenceToQuestion` became dead once their only callers (`handleSqlEditorToggle`/`handleAddReference` in QuestionViewV2.tsx) were deleted in this same PR — completed that cleanup.
- **8 dead types** in the `lib/types.ts` barrel cluster: `ChatMessage`, `ToolCallRound`, an entire superseded eval-types cluster (`EvalAppState`/`BinaryAssertion`/`NumberAssertion`/`EvalAssertion`), `QuestionContainer` (the type, not the component), `SchemaInfo`.
- **A dead connector config cluster** in `lib/connections/base.ts` (9 per-dialect interfaces + `ConnectorConfigMap`) — zero real consumers; `NodeConnector.config` is typed `Record<string, any>` throughout. Its own doc (`ADDING_A_CONNECTOR.md`) referenced a `ConnectorDialect` type that doesn't exist and stale pre-M3 line numbers — rewrote the relevant section to describe the real mechanism (`lib/types/connections.ts`'s `DatabaseConnection.type` union + `connectionTypeToDialect`).
- **Smaller items**: `STEP_LABELS`, `TOKEN_REFRESH_THRESHOLD`, `signIn`/`signOut` unused re-exports, and a partial-dead test-harness module (`resetDB`, `ensureMxfoodDataset`, `addMxfoodConnection`, `faux`/`getLLMReceived`/`assertLLMReceived`/`RecordedLLMCall` — kept the live siblings `addTestConnection`/`setFauxLLM`/`resetFauxLLM`). Two of these (`STEP_LABELS`, `TOKEN_REFRESH_THRESHOLD`) reverses a same-day M1 triage decision on `main` that, on closer reading, turned out to rest on a misread comment (descriptive, not a real "kept for future" signal) — corrected rather than left as a stale precedent.
- **Doc staleness**: PR #567's actual merged status (the doc said "not merged" after the owner had in fact merged it), a stale `piLogToLegacy` rationale in `chat-architecture-v3.md`, a CLAUDE.md completeness gap re: Slack's in-process path, one stale test comment.

**`frontend/lib/chat-v2/` (the `?v=1` legacy-chat toggle) — initially kept, then deleted on owner confirmation.** Looked unreachable in the current data flow (every write-site hardcodes `version: 3`), but the backfill migration is manual/admin-triggered, not automatic, so the initial call was to keep it as a plausible compatibility feature for un-migrated self-hosted deployments — same reasoning pattern as the M6 "looks abandoned but is load-bearing" findings. The owner confirmed the migration is in fact complete everywhere it needs to be and the toggle is unneeded — removed entirely: the module itself, the "Use new chat interface" Alpha Flags toggle, the legacy-chat banner in `ChatInterface.tsx` (the message input is no longer conditionally hidden), the `v`-param propagation surgically removed from `url-utils.ts`/`create-middleware.ts`/`fetch-patch.ts`/`store/api-url.ts` (each also handles `as_user`/`mode`, left untouched), 5 dedicated test files, and one cascade (`unpatchFetch`, whose only consumer was one of those deleted tests). `conversations-page.ui.test.tsx`'s 2 tests were rewritten to assert `as_user`/`mode` preservation (what the page's navigation still needs) instead of the removed `v` propagation, rather than just deleted, since the underlying real-integration-test scaffolding was still valuable. Browser-verified: Settings → Developer Tools → Alpha Flags now shows only "Queue Strategy," no errors.

Two transient infra crashes during this round (both "Connection closed mid-response") were recovered without redoing work: one worktree had real salvageable progress (committed before resuming); one agent correctly self-detected it had been given a worktree based on `main` instead of `feature/refactor-v3` and stopped rather than force a broken deletion — the affected items were redone directly, correctly scoped, once the branch mismatch was understood.

All 17 GitHub Actions checks pass against the final pushed commit (verified via `gh pr checks 568` against the exact head SHA, not a stale run). Not merged, per standing instruction.

## Next PR — DONE (2026-07-06) — PR #568, branch `feature/refactor-v3`

Both parts below shipped in one PR as decided. Executed as 3 parallel worktree-isolated agents (Slack; QuestionViewV2; DashboardView), each reviewed diff-by-diff and independently re-tested before merging (not just trusted on report), merged sequentially into `feature/refactor-v3`, followed by the scoped ESLint guard, a fresh knip sweep, and full validation (build, all 3 Vitest projects, e2e, QA, live browser pass). PR #568 opened by the owner; not merged, per standing instruction.

**What actually shipped, vs. what was scoped below:**
- **Slack migration was narrower than scoped** (good news): re-verified against `main` before starting and found benchmark-import + connection-wizard onboarding were *already* v3 (confirmed independently, matching what this doc already said) — Slack's own gap was narrower too: it was already writing to v3 `conversations`/`messages` tables, it just drove its own bespoke orchestration loop (`runChatOrchestrationV2`) instead of the shared `runConversationTurn` turn-runner, so it had persistence but not lease/heartbeat, LISTEN/NOTIFY, or error-stream mirroring. That's what got fixed. `runChatOrchestrationV2` + `lib/chat/run-orchestration.server.ts` deleted entirely. `docs/chat-architecture-v3.md` updated to say v2 is fully removed.
- **QuestionViewV2's move was blocked once, then unblocked with expanded scope**: the assigned agent discovered (correctly, and stopped rather than force it) that `QuestionViewV2` has **4 real mount sites**, not 1 — `QuestionContainerV2`, `CreateQuestionModalContainer` (a live edit-mode-on-mount flow), and two chat-tool-display components (`Markdown`'s `InlineChart`, `ExecuteQueryDisplay`). Scope was expanded (owner-approved implicitly via "do it fully") to migrate all 4 callers rather than leave 3 broken. Side effect: fixed a latent bug where the two toolcall/read-mostly callers used to read the *global* panel-collapse Redux slice, meaning a concurrently-open page view could leak state into an inline chat chart's button label — they now use local state instead.
- **DashboardView's move went exactly as scoped** — confirmed a single real mount site (`DashboardContainerV2`), Blue→Red→Blue executed cleanly (RED-proven by breaking 3 independent call sites one at a time and confirming scoped failures).
- **Dead-code sweep found one genuinely new item**: `components/modals/QuestionPickerModal.tsx`, orphaned by the QuestionViewV2 move (confirmed independently by the agent's grep and by knip) — deleted. Nothing else in the fresh knip sweep was new; everything else matched the already-triaged M1/Final-validation baseline (barrel re-exports, the `lib/types.ts` split, `agents/benchmark-analyst/v2/*` intentional-keep, `tailwindcss`/`yaml-loader` config-only devDeps).
- **ESLint guard**: added scoped to just the 2 files fixed this round (`QuestionViewV2.tsx`, `DashboardView.tsx`), not repo-wide — the other 9 M4.2 candidates (`ConnectionFormV2`, `TransformationView`, `AlertView`, `ReportView`, `NotebookView`, `CodeView`, `StoryView`, `AgentHtml`, `InlineNumber`) still violate the convention and remain future work. RED-proven (temporarily reintroduced a `@/store/hooks` import, confirmed the lint error, reverted).

**Process note — two transient agent crashes, both recovered without redoing work:** both the DashboardView and QuestionViewV2 continuation agents hit "API Error: Connection closed mid-response" mid-flight (infra-level, unrelated to the code). Per this project's established liveness protocol, neither was blindly re-dispatched or trusted — each worktree's actual git state was checked directly first. DashboardView had real, passing, uncommitted progress (committed before resuming). QuestionViewV2's had zero uncommitted changes (nothing lost). Both were resumed via the same agent (not a fresh one) so prior context/reading wasn't repeated.

---

### Part 1 (as originally scoped) — finish the chat v2→v3 migration (Slack), then delete all v2 chat code + docs
...
- **Connection-wizard onboarding is already v3** — `StepGenerating.tsx`/`StepContext.tsx` both POST to `/api/conversations` with `version: 3`.
- **Slack is the only remaining v2 consumer.** `lib/integrations/slack/process-event.ts` calls `runChatOrchestrationV2` (defined in `lib/chat/run-orchestration.server.ts`) for every inbound Slack event. This is the one real gap, not three.

So `docs/chat-architecture-v3.md`'s v2-retention rationale is itself now stale for 2 of its 3 listed cases — update it as part of this PR once Slack lands, rather than now (its Slack claim is still accurate today).

**Reference implementation exists but is not mergeable.** `origin/feature/chat-arch-v3` (the branch PR #513 shipped from) kept receiving commits after #513 merged that were never opened as a second PR: `6b64cd73` (Stage A: migrate headless chat — Slack + benchmark — to v3 store), `af00887c` (onboarding wizard uses v3; commit message: *"With this, v2 has no remaining app consumers"*), `e3dc9fe2`/`a20dace9` (Stage C+D/E: delete all v2 chat server + client code), through `5bf0440c` (unify all chat on v3, drop the file-conversation surface) as the branch's last commit. That branch fully finished this exact job — it just never merged. It is now 59 commits behind `main` (`git log --oneline origin/feature/chat-arch-v3..main`) and predates this refactor's restructuring of `lib/chat/*`, so it is **not** safe to merge or cherry-pick. Use `git diff main...origin/feature/chat-arch-v3 -- frontend/lib/integrations/slack frontend/lib/chat` (and read the individual commits above) as a reference for what changed, then re-implement equivalent changes by hand against current `main`.
...
### Part 2 (as originally scoped) — unblock M4.2 (container/view) for `QuestionViewV2`/`DashboardView`
