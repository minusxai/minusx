...
import { DatabaseWithSchema, ContextVersion, DocEntry, MetricDef, TableAnnotation, SkillEntry, Whitelist, SemanticModel } from '@/lib/types';
...
export async function computeSchemaFromWhitelist(
  ...
  if (isRoot) {
    ...
    return { fullSchema, parentSchema: allConnections, fullDocs: [], fullMetrics: [], fullAnnotations: [], fullSemanticModels: [], fullSkills: [] };
  }
  ...
  const fullSemanticModels = [...(ancestorContent.fullSemanticModels || []), ...(publishedVersion.semanticModels || [])];

  // parentOffering = what the parent makes available to this context (before own whitelist)
  return { fullSchema, parentSchema: parentOffering, fullDocs, fullMetrics, fullAnnotations, fullSemanticModels, fullSkills };
}
