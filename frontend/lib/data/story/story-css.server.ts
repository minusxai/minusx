...
export function buildAppThemeCss(): string {
  // Unthemed dashboards must keep TODAY'S chart colors (the visual bar — Renderer_v2 Phase 3):
  ...
  // COLOR_PALETTE head) into the neutral host blocks; [data-theme] blocks still override.
  const appCharts = COLOR_PALETTE.slice(0, 5);
  const withAppCharts = (body: string): string =>
    body.replace(/--chart-([1-5]): [^;]+;/g, (_m, n: string) => `--chart-${n}: ${appCharts[Number(n) - 1]};`);
...
${SHADCN_THEME_MAPPING}
:root ${SHADCN_NEUTRAL_LIGHT_BODY}
.dark ${SHADCN_NEUTRAL_DARK_BODY}
`;
