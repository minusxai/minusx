...
class FilesDataLayerServer implements IFilesDataLayer {
  ...
  async batchSaveFiles(files: Array<{
    ...
    // Save each file with same validation as saveFile
    for (const file of files) {
      // Get existing file
      const existingFile = await DocumentDB.getById(file.id, user.companyId);

      if (!existingFile) {
        throw new FileNotFoundError(file.id);
      }

      // Check file access (unified: type + mode + path)
      if (!canAccessFile(existingFile, user, overrides)) {
        throw new AccessPermissionError(`You do not have permission to modify file ${file.id}`);
      }

      // Check if path is protected
      if (PROTECTED_FILE_PATHS.includes(file.path as any)) {
        throw new AccessPermissionError(`Cannot modify file at protected path: ${file.path}`);
      }
      ...
      // For connections: strip schema (server-managed field)
      if (existingFile.type === 'connection') {
        const { schema, ...connectionContentWithoutSchema } = file.content as ConnectionContent;
        contentToSave = connectionContentWithoutSchema as BaseFileContent;
      }

      // For contexts: strip fullSchema and fullDocs (server-computed fields)
      if (existingFile.type === 'context') {
        const { fullSchema, fullDocs, ...contextContentWithoutComputed } = file.content as ContextContent;
        contentToSave = contextContentWithoutComputed as BaseFileContent;
      }

      // For questions: compute and store queryResultId
      if (existingFile.type === 'question') {
        const questionContent = file.content as QuestionContent;
        // Build params object from parameter values
        const params = (questionContent.parameters || []).reduce((acc, p) => {
          acc[p.name] = p.value ?? '';
          return acc;
      ...
      // Update in database
      const success = await DocumentDB.update(file.id, file.name, file.path, contentToSave, file.references, user.companyId);

      if (!success) {
        throw new Error(`Failed to save file ${file.id}`);
      }
