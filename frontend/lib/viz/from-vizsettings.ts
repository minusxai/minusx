 ...
 *
 * Styling is best-effort: columnFormats/conditionalFormats flow through to the sources
 * that carry them (table/pivot/recipe); the remaining V1 style knobs (styleConfig,
 * axisConfig, annotations) are carry-over gaps (§21 items 6/9), not blockers.
 */
...
export function vizSettingsToEnvelope(
  ...
  switch (type) {
    ...
    case 'row':
    case 'pie':
      return cartesianEnvelope(vizSettings, columns, type as SpecVizType);
