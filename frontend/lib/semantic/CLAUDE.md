...
`childPaths` entries are **relative to the granting context's folder** (`resolveChildPath` in `lib/sql/schema-filter.ts`; a leading `/` is the legacy absolute form, still resolved, so no migration). The loader absolutizes them exactly once, when an authored version's entries are lifted into the computed `full*` plane — deeper levels re-check against absolute copies without needing the author's dir. On a **folder move or rename**, `FilesAPI.moveFile` rewrites `childPaths` in exactly two bounded sets of contexts (`rewriteChildPathsForMove`, `lib/context/context-utils.ts` — a deep walk keyed on the property name, so whitelist nodes at any nesting level, docs, views and semantic models are covered without enumerating them): contexts *inside* the moved subtree (relative entries pass through untouched; legacy absolute ones are prefix-rewritten), and *common-ancestor* contexts — a grant follows the folder for as long as it stays inside the grantor's subtree, which is what keeps grants working across renames. A grant whose target *leaves* the grantor's subtree is left byte-identical and simply stops applying. Path rewrites and content rewrites apply as one SQL statement (`DocumentDB.applyFolderMove`), so a failure rolls the entire move back on every backend without client-side transaction machinery.
