...
describe('waitForFileViewReady', () => {
  ...
  it('resolves quickly when the view exists and nothing is busy', async () => {
    addFileView(1);
    const t0 = Date.now();
    await waitForFileViewReady(1, { timeoutMs: 3000, settleMs: 50, pollMs: 10 });
  ...
  it('resolves at the timeout even if a busy marker never clears (never hangs the screenshot)', async () => {
    ...
    const t0 = Date.now();
    await waitForFileViewReady(4, { timeoutMs: 300, settleMs: 50, pollMs: 20 });
