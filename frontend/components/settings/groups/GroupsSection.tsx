...
interface Group {
  ...
  folders: string[]; memberIds: number[];
}
...
interface Draft { originalName?: string; name: string; preset: PresetKey; folders: string[]; memberIds: number[] }

function toDraft(g: Group): Draft {
  return { originalName: g.name, name: g.name, preset: presetOf(g), folders: g.folders, memberIds: g.memberIds };
}
function emptyDraft(): Draft {
  return { name: '', preset: 'view', folders: [], memberIds: [] };
}
...
export function GroupsSection() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = { name: draft.name.trim(), allowedTypes: preset.allowedTypes, viewTypes: preset.viewTypes, createTypes: preset.createTypes, folders: draft.folders, memberIds: draft.memberIds };
      const res = draft.originalName
        ? await fetch(`/api/groups/${encodeURIComponent(draft.originalName)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(payload) })
      ...
      toaster.create({ title: draft.originalName ? 'Group updated' : 'Group created', type: 'success' });
  ...
  const remove = async (g: Group) => {
    const res = await fetch(`/api/groups/${encodeURIComponent(g.name)}`, { method: 'DELETE', credentials: 'include' });
  ...
  const addScope = () => {
    ...
    if (f && draft && !draft.folders.includes(f)) setDraft({ ...draft, folders: [...draft.folders, f] });
    setScopeInput('');
  };
  ...
  return (
    <VStack align="stretch" gap={5} aria-label="Groups settings">
      ...
      <VStack align="stretch" gap={2}>
        {groups.map(g => (
          <Flex key={g.name} align="center" justify="space-between" p={3} borderWidth="1px" borderColor="border.default" borderRadius="md" aria-label={`Group ${g.name}`}>
            <Box>
              <HStack gap={2}>
              ...
              <Text fontSize="xs" color="fg.muted" fontFamily="mono" mt={1}>
                {g.folders.length ? g.folders.map(f => `/${f || ''}`).join(', ') : 'no folders'} · {g.memberIds.length} member{g.memberIds.length === 1 ? '' : 's'}
              </Text>
            </Box>
            <HStack gap={1}>
      ...
      {draft ? (
        <Box p={4} borderWidth="1px" borderColor="accent.primary/40" borderRadius="md" aria-label="Group editor">
          <VStack align="stretch" gap={3}>
            <Box>
              <Text fontSize="xs" fontWeight="600" mb={1}>Name {draft.originalName ? <Text as="span" color="fg.muted">(names are permanent)</Text> : null}</Text>
              <Input aria-label="Group name" value={draft.name} disabled={!!draft.originalName} onChange={e => setDraft({ ...draft, name: e.target.value })} placeholder="finance-viewers" fontFamily="mono" />
            </Box>
            <Box>
              ...
              <HStack gap={1} mb={2} flexWrap="wrap">
                {draft.folders.map(s => (
                  <Badge key={s} colorPalette="blue" aria-label={`Scope ${s}`}>
                    /{s}
                    <Box as="button" ml={1} aria-label={`Remove scope ${s}`} onClick={() => setDraft({ ...draft, folders: draft.folders.filter(x => x !== s) })}><LuX size={11} /></Box>
                  </Badge>
                ))}
              </HStack>
              <HStack gap={2}>
