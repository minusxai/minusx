...
function parseMarkdown(markdown: string) {
  ...
  // Image/link nodes nest inside paragraphs, so collect descendant types too.
  ...
  editor.getEditorState().read(() => {
    ...
    const collect = (node: any) => {
      allNodeTypes.push(node.getType());
      if (typeof node.getChildren === 'function') node.getChildren().forEach(collect);
    };
    children.forEach(collect);
  });

  return { editor, nodeTypes, nodeDetails, allNodeTypes };
}
...
describe('markdown-transformers', () => {
  ...
  it('round-trips an image, preserving alt text and src', () => {
    const md = '![a cat](https://example.com/cat.png)';
    const result = roundTrip(md);
    console.log('Image round-trip:', JSON.stringify(result));
    expect(result).toContain('![a cat](https://example.com/cat.png)');
  });

  it('prefers the image transformer over the link transformer for ![...](...)', () => {
    const { allNodeTypes } = parseMarkdown('![alt](https://example.com/x.png)');
    expect(allNodeTypes).toContain('image');
    expect(allNodeTypes).not.toContain('link');
  });
});
