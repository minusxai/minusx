...
- a jsx field (e.g. a story's HTML body) is emitted INLINE as real elements, with \`<Question id={N}/>\` embeds: \`<story><div class="story">…<Question id={5}/>…</div></story>\`. A story can also declare shared filters with \`<Param name="city" type="text" nullable={false} id={5} />\` — every embedded question's matching \`:param\` binds to it (\`id={N}\` autocompletes/imports from question N's column). If an embedded question uses a \`:param\` with no \`<Param>\`, you'll get a validation warning (non-blocking).
