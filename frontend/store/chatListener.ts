...
import { selectAllowChatQueue, selectQueueStrategy, selectDevMode, setDevMode, setBulkUiFlags } from './uiSlice';
...
import { runV3Turn, observeConversation, type V3TurnInput } from './conversation-stream-client';
import { ConversationsAPI } from '@/lib/data/conversations';
import { loadConversationDetail, invalidateConversationLogCache } from './conversation-log-cache';
 ...
 */
/** Wire view for conversation reloads: verbatim log in dev mode, slim projection otherwise. */
const viewFor = (state: RootState): ConversationView => (selectDevMode(state) ? 'full' : 'display');
...
async function runV3TurnInListener(
  ...
  // A retry truncates-and-replays the dead turn's rows server-side — the cached tail is stale.
  ...
  // generic "chat error". An errored turn skips the incremental path: its partial rows may be
  // truncated by the upcoming retry.
...
async function renderFromDurableLog(
  ...
  const detail = await loadConversationDetail(conversationID, view);
  const piLog = detail.piLog;
  const errors = detail.errors.map((e) => ({
    source: e.source, message: e.message,
    timestamp: Date.parse(e.createdAt) || Date.now(),
...
// One observer per conversation per tab (a re-mint re-dispatches setRemoteSession; don't stack).
// eslint-disable-next-line no-restricted-syntax -- client-side Redux listener; per-tab ephemeral observer registry
const observingConversations = new Set<number>();

/**
 * setRemoteSession(active) → passively tail the externally-driven conversation: committed remote
 * tool activity renders live (streamed rows), `pending` events set pending_tool_calls (→ the
 * existing auto-exec machinery executes the frontend tool and the completeToolCall listener's
 * remote branch POSTs the result append-only). Ends when the session does (stream turns terminal).
 */
chatListenerMiddleware.startListening({
  actionCreator: setRemoteSession,
  effect: async (action, listenerApi) => {
    const { conversationID, active } = action.payload;
    if (!active) return;
    if (IS_TEST) return; // no XHR/SSE in jsdom — node tests drive updateConversation directly
    if (observingConversations.has(conversationID)) return;
    observingConversations.add(conversationID);

    const dispatch = listenerApi.dispatch as AppDispatch;
    const state = listenerApi.getState() as RootState;
    const conversation = selectConversation(state, conversationID);
    if (!conversation) { observingConversations.delete(conversationID); return; }
    ...
    const reloadNow = () => enqueue(async () => {
      ...
      if (cur) await renderFromDurableLog(conversationID, cur, dispatch, 'EXECUTING', viewFor(listenerApi.getState() as RootState));
    });
    ...
    try {
      ...
      if (!(error instanceof Error && error.name === 'AbortError')) {
        console.warn('[remote-session] observer stream failed:', error);
      }
    } finally {
      ...
      if (latest) {
        try { await renderFromDurableLog(conversationID, latest, dispatch, 'FINISHED', viewFor(listenerApi.getState() as RootState)); } catch { /* offline */ }
      }
...
chatListenerMiddleware.startListening({
  ...
  effect: async (action, listenerApi) => {
    ...
    // Remote Agent Session: the browser only ever posts frontend-tool COMPLETIONS — the turns
    // route short-circuits them append-only (no LLM resume; the remote endpoint's waiter serves
    // the result to the external agent). Clear pending; the observer stream renders what follows.
    if (conversation.remoteSession?.active) {
      const completed_tool_calls = conversation.pending_tool_calls.map(p => [p.toolCall, p.result!]);
      try {
        const res = await fetch(patchApiUrl(`${API_BASE_URL}/api/conversations/${conversationID}/turns`), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ completedToolCalls: completed_tool_calls }),
        });
        if (!res.ok) throw new Error(`remote completion failed: HTTP ${res.status}`);
      } catch (error) {
        console.error('[remote-session] failed to post tool completion:', error);
        reportClientErrorToServer(conversationID, error instanceof Error ? error.message : String(error));
      }
      dispatch(updateConversation({
        conversationID,
 ...
 * The raw-log cache must never mix slim and full entries, so any toggle
 ...
 * setAsActive: false.)
 *
 * Fires on BOTH the explicit toggle (setDevMode) and the localStorage restore at boot
 * (DataLoader's setBulkUiFlags): page-level effects run before the app-shell layout effect, so
 * a dev-mode cold load can fetch the slim view first — the restore then upgrades it to full.
 */
async function onDevModeEnabled(enabled: boolean, listenerApi: { getState(): unknown; dispatch: unknown }): Promise<void> {
  invalidateConversationLogCache();
  if (!enabled) return;
...
chatListenerMiddleware.startListening({
  ...
  effect: async (action, listenerApi) => onDevModeEnabled(action.payload, listenerApi),
});

chatListenerMiddleware.startListening({
  actionCreator: setBulkUiFlags,
  effect: async (action, listenerApi) => {
    if (action.payload.devMode === undefined) return;
    await onDevModeEnabled(action.payload.devMode, listenerApi);
  },
});
