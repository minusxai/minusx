...
| `store/configsSlice` ← SSR `preloadedState` | env → us | `MAX_CONCURRENT_QUERIES` and `QUERY_TIMEOUT_MS` reach `querySemaphore` and the fetch timeout through Redux, read live on each acquire. `MX_EGRESS_IPS` arrives the same way as `egressIps`, and is passed down as a *prop* (never a hook) because `ConnectionFormV2` is Redux-restricted. |
...
- **Viz must never deep-merge.** `editFile` writes a Viz V2 envelope whole; merging deltas resurrects deleted encoding channels and mangles spec arrays.
