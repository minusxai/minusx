...
/** Auto-generated UI-only messages (e.g. preloaded skills). Ignored by LLM message reconstruction. */
export type AutoGeneratedMessage = {
  role: 'autogenerated';
  type: string;               // e.g. 'skill_load'
  content: Record<string, any>;  // type-specific payload
  created_at: string;
};

/** Error row sourced from conversation.errors[] (cycles 1-5). Never sent to the
 *  LLM; rendered distinctly by SimpleChatMessage. */
...
export interface Conversation {
  ...
  messages: Array<UserMessage | CompletedToolCall | DebugMessage | AutoGeneratedMessage>;
  ...
  // AI-generated conversation title (shown in the chat header). Undefined until a
  // title is generated server-side (after the first turn). Loaded with the
  ...
  remoteSession?: { active: boolean; expiresAt?: string };
}
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    // Remote Agent Session flag — set on mint / cold-load of a remote conversation; cleared when
    // the session ends (Stop, expiry, agent /end). Clearing also drops any pending frontend tools
    // (the server already closed them with isError results) and returns the UI to FINISHED.
    setRemoteSession(state, action: PayloadAction<{ conversationID: number; active: boolean; expiresAt?: string }>) {
      const { conversationID, active, expiresAt } = action.payload;
      const conv = state.conversations[conversationID];
      if (!conv) return;
      conv.remoteSession = { active, ...(expiresAt ? { expiresAt } : {}) };
      if (!active) {
    ...
    // Append auto-generated UI-only messages (ignored by LLM pipeline)
    addAutoGeneratedMessages(state, action: PayloadAction<{
      conversationID: number;
      messages: AutoGeneratedMessage[];
    }>) {
    ...
    setConversationTitle(state, action: PayloadAction<{ conversationID: number; title: string }>) {
      const conv = state.conversations[action.payload.conversationID];
