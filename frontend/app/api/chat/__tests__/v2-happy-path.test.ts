...
//   5. The frontend never sees pi-ai shape — the response is in legacy
//      ChatResponse shape with debug entries derived from pi-ai usage.
...
describe('POST /api/chat?v=2 — happy path (orchestrator runs, response is legacy shape)', () => {
  ...
  it('first turn: creates v=2 conversation, runs orchestrator, returns legacy ChatResponse, renames file, persists pi-ai log', async () => {
    ...
    // (translated from the assistant text), NOT pi-ai entries.
    ...
    // Debug entries derived from pi-ai usage. The faux LLM auto-computes
    ...
    // and the on-disk content.log is the pi-ai shape (not legacy).
    ...
    // Pi-ai shape: root invocation has `type: 'toolCall'`, assistant has `role: 'assistant'`.
