...
- Known UX gap (small, deliberate): the Copy-to-Agent button lives in the chat header, which renders only once the conversation has ≥1 message — an empty side chat can't mint. Follow-up candidate: surface the button for empty conversations too.
