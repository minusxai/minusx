...
//       browser (body: { error: 'browser_unreachable' }) — see §8 B10
```
...
| E5 | **User closes the tab mid-session** | Server tools keep working (no browser needed). A frontend-bridged call times out → 410 `browser_unreachable` + `isError` toolResult (§8 B10); the session itself stays live until idle/TTL expiry, so reopening the tab reattaches the observer (E6) and the agent can retry the tool. |
...
### Phase 1 — Sessions & skill doc (server foundation)
- [ ] **Contracts**: `lib/data/remote-sessions.types.ts` (§4.1), `RunStatus` union extension, HTTP types (§4.3); verify no DB CHECK constraint on `run_status`.
- [ ] **Red tests** (`node` project): code encode/decode + hash verify (timing-safe; tampered/expired/revoked/idle cases); mint/revoke/status routes (owner-only; mint guard per §4.4 — 409 on `running`/`paused`; re-mint revokes prior); `withRemoteSessionAuth` (404 uniformity, `lastActivityAt` bump, lazy expiry release + pending-call cleanup); `GET /s/<code>` skill doc (contains toolset schemas + protocol; serves "session ended" after revoke); middleware admits `/s/` unauthenticated; **fork strips `meta.remoteSession`** (§9.4 E11).
- [ ] **Implement**: `lib/data/remote-sessions.server.ts`, `lib/http/with-remote-session-auth.ts`, `app/api/conversations/[id]/remote-session/route.ts`, `app/s/[code]/route.ts`, middleware admit-list entry, root-invocation append on mint.
- [ ] Full suite green (`npm test`), `npm run validate`, push.

### Phase 2 — Remote tool execution (the engine)
- [ ] **Contracts**: `RemoteToolCallRequest/Completed/Pending`, `serializeRemoteContent`, `REMOTE_REGISTRABLES` (leaf-only, minus `ClarifyFrontend`), extracted `buildAgentContext` (Blue→Red→Blue: characterization tests on `setupOrchestration` first, then extract, then re-green).
- [ ] **Red tests** (`node` + `orchestrator` projects): dispatch-driven execution of a server tool (faux-free — real `ExecuteQuery` against test DuckDB) appends well-formed assistant+toolResult rows **and a later normal faux-LLM turn on the same conversation still loads/runs** (log-invariant regression — the most important test in the feature); unknown tool / bad args / agent-name / `ClarifyFrontend` → 400; frontend tool → pending row + 202 → simulated browser `completedToolCalls` POST (through the real turns route) unblocks the waiter → poll returns the result; short-circuit does NOT invoke the LLM (assert faux provider never called); completion dedupe by `toolCallId` (§5.3); single-flight 409; turns route rejects `userMessage`/`autoRetry` while `remote` (§9.4 E1/E12); browser-timeout 410 appends `isError` result; Stop mid-call appends `isError` + wakes waiter (§9.4 E2); base64 inlining of local serve-route URLs.
- [ ] **Implement**: `app/s/[code]/tool/route.ts`, `…/result/[toolCallId]/route.ts`, `…/context/route.ts`, `…/end/route.ts`, waiter util (`lib/chat/remote-tool-waiter.server.ts`), turns-route short-circuit + guards, `'remote'` threading (§8 B6 sites), `AppEvents.REMOTE_TOOL_CALL` + rate limit.
- [ ] Full suite green, validate, push.

### Phase 3 — Frontend (observer + freeze + button)
- [ ] **Red tests** (`ui` project, aria-label queries only; `node` for the listener flow): observer dispatches on remote pending → auto-exec runs handler → `completedToolCalls` POSTed (extend the `agent-e2e.ui.test.tsx` pattern, remote-driven pending injected via real stream events); observer re-attaches on load when `runStatus === 'remote'` (§9.4 E6); `ChatInput` hard-locks when `remoteSession.active` (ignores `allowChatQueue`); Copy-to-Agent button disabled while agent running (§9.4 E4); banner renders + Stop calls DELETE + clears on `idle`; mint button writes `copyText` to clipboard.
- [ ] **Implement**: `observeConversation` client mode + listener wiring, `chatSlice` flag/actions, `ChatInput`/`ChatInterface` gates + banner, `ChatHeaderBar` button (with `aria-label`).
- [ ] Full suite green, validate, push.
- [ ] Manual browser-verify with a real external agent (§13.2).
