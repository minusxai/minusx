...
### Phase 3c — Third browser round: the target workflow, smooth (user-directed)
The canonical workflow — **dashboard side chat → mint → agent creates a story from the dashboard → edits it repeatedly** — run end-to-end with zero explore-page involvement and zero friction:
- [x] **Navigate is auto-allowed in remote sessions** (red→green test `lib/tools/__tests__/navigate-remote-session.test.ts`): the user granted STANDING consent by minting (banner + Stop always visible; the session is the sole driver per §9.4), and agent workflows navigate on every file hop — per-hop confirms made the flow unusable. Normal chat still confirms every navigation. Browser-verified: `Navigate newFileType:"story"` completed in 0.45 s with no prompt.
- [x] **Skill doc now sets up the agent properly**: a "First: confirm the goal with YOUR user" section (connect → orient read-only → ask the human what to do BEFORE changing anything), an explicit `LoadSkill` instruction for per-file-type markup guides (`stories`, `questions`, `dashboards`, …), and the create-new-file recipe (Navigate `newFileType` → EditFile → PublishAll-with-user-review). Verified live: `LoadSkill("stories")` returns the full authoring guide in-process.
- [x] **Side-chat-only run verified in the browser**: dashboard side chat mint → `ReadFiles` dashboard → unprompted `Navigate` to a new story draft → three successive `EditFile`s (body, headline rewrite, new section) — the story page updated live each time, and the side chat ON the story page showed the session (turn boundary, tools timeline, "3 FILE EDITS #1087", banner + Stop, frozen input) because the active conversation follows across pages.
- [x] **FIXED (was "known gap"): an empty side chat can now mint.** The chat header renders on empty conversations too, and Copy-to-Agent creates the conversation on demand (same lazy pre-create the send path uses: `POST /api/conversations` → `createConversation` in Redux → mint on the new id). Red→green ui test; browser-verified: fresh side chat → one click → session live (root turn + banner + frozen input), skill doc served, server tool ran.

### Phase 3d — Mint-time app state (user-directed)
- [x] **The session now knows what page the user is on** (was a gap: mint was a bare POST, so the agent had no idea the user was looking at a dashboard). The client sends the page's app state with the mint (`ChatInterface`'s `appState` prop → `ChatHeaderBar` → mint body, bounded server-side via `boundContextAppState` like the turns route); it lands on the session root invocation's `context.appState`/`pageType` (exactly what a normal turn carries, so tools and later LLM turns see it), and a lean page summary (`RemoteSessionRecord.page`) surfaces as a "Current page" line in the skill doc and `currentPage` in `/context` ("the user is looking at dashboard "Top Level Metrics" (file id 11) — ReadFiles it for details"). Re-mint preserves the prior page when no fresh app state is sent. Red→green route tests; browser-verified from the dashboard side chat.

### Phase 3e — Feature toggle, prod-URL parity, honest session-ended, live-traffic learnings (user-directed)
- [x] **"Remote Agents" Settings toggle, OFF by default** (Settings → Integrations, `OrgConfig.remoteAgentsEnabled`, per-mode like all org config): gates the Copy-to-Agent button (client), minting (403), AND live-code auth (toggling OFF kills active links immediately). Red→green tests at every layer; found+fixed a latent `mergeConfig` bug on the way (explicit key enumeration — the CLAUDE.md code smell — silently dropped new config keys from the client-merged config).
- [x] **Base-URL prod parity**: the copied link is now composed CLIENT-side from `window.location.origin` — the same mechanism public link sharing uses (its API returns a relative path; the client composes) — so it is correct on any deployment; the server's absolute URL (now honoring `x-forwarded-host`) remains for API consumers, and the skill doc self-references from the agent's own request headers (correct by construction).
- [x] **Honest "session ended"**: a dead code that still PROVES ownership (nonce matches the stored hash: stopped/expired/disabled) now gets `410 {error:"session_ended"}` from tool endpoints (matching the skill-doc page); unknown/malformed/replaced codes stay uniform 404 (indistinguishable from guesses — no security loss). Live agent traffic showed the bare 404 confused agents.
- [x] **Markup-normalization guidance** (from live agent traffic): stored markup is a normalized projection (comments dropped, attributes rewritten) — the skill doc now warns agents to anchor `oldMatch` only on markup they have SEEN (diff / `currentMarkup` / ReadFiles), never on text authored from memory. (EditFile already diffs against the stored form and flags normalization; the agent just needs the rule up front.)
- Live-traffic note (no code change): an agent "stuck pondering" was its own `sleep 150` pacing while polling a genuinely slow browser-side chart query; the 202→poll protocol worked as designed. The skill doc advertises `?waitMs` server-side long-poll as the better pattern.
- [x] **"How to work in MinusX" rules added to the skill doc** (staging traffic, Starlight run): a remote agent replaces our LLM, so it never sees the ANALYST SYSTEM PROMPT — the platform's working culture (reuse existing questions, story-not-dashboard for narratives, LoadSkill before authoring, live `<Number>` embeds never hardcoded metrics, draft-vs-saved semantics) simply didn't transfer. A real run created duplicate questions, built a "story-style dashboard", authored story JSX blind (two rewrite cycles), and hand-typed metrics. The skill doc now carries those five rules as non-negotiables. Also fixed: `CheckFileHealth`'s description now states it grades the SAVED version only (a fresh draft scores 0/5 — the agent hit exactly this and burned calls diagnosing it); draft feedback = EditFile's built-in review.
- [x] **Ask-first tightened** (staging traffic): the doc's "read-only orientation calls are fine up front" license led a real agent to run 9 orientation calls (ReadFiles, SearchDBSchema, …) BEFORE asking its user, even though the doc already carries the current page + connections. Rewritten: ask FIRST with zero tool calls (the doc contains everything needed for the question); orient only after the goal is known, and only as much as the task needs.
