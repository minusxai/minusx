...
- Known gap (pre-existing, NOT remote-specific, deliberately out of this PR's scope): the Publish modal's **Save All on a NEW draft** (provisional path) requires a save-location selection that the header Save flow prompts for but the modal flow doesn't surface smoothly — the user had to pick the location manually. Affects our own LLM agent's `PublishAll` on new drafts equally. Follow-up: default provisional-path drafts to their draft parent folder in the modal (or surface the location picker).
