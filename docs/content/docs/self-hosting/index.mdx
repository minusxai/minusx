...
2. **Telemetry** — crash reporting is on by default and product analytics may be enabled by default in official images; both are disabled by a single environment variable. The full, honest inventory is on the [Telemetry](/docs/self-hosting/telemetry) page.
