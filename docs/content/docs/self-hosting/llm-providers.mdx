...
The recommended starting points — this table renders from the same [compatibility.json](https://github.com/minusxai/minusx/blob/main/frontend/compatibility.json) the `install.sh` interview reads. The interview offers the bring-your-own-key providers below; the managed MinusX provider is configured in the app rather than during install:
