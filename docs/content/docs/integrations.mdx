...
Six tools are available to any connected MCP client:
...
| **LoadContext** | Read documents from your Context Library (offered only when your context has library docs) |
...
Both are available at **Settings → Integrations → Slack**, and the setup wizard's **Slack** step
offers whichever applies to your instance.
...
Available when the deployment has `SLACK_CLIENT_ID` and `SLACK_CLIENT_SECRET` set. Self-hosted
installs typically do not, and use manual setup instead.
...
Manual setup requires MinusX to be served over a **public HTTPS URL** — Slack delivers events over
the public internet, so it cannot reach an instance on `localhost` or a private address. Until one
is configured, both setup options are hidden and MinusX says so rather than offering a flow that
cannot complete. In development, an HTTPS tunnel (e.g. ngrok) satisfies this.
