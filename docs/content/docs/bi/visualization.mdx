...
ships built-in recipes — bullet, lollipop, range bar, **radar** and **heatmap** — available in
every folder without setup, and you (or the agent) can author new ones.

- **Seeing what ships** — **Templates › Visualizations** lists every built-in and shipped chart
  recipe. Open one to see a live preview, its binding slots and its template. They are read-only,
  and each copyable one has a **Copy to my workspace** button that gives you an editable version
  to change or override.
