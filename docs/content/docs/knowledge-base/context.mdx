...
The main sidebar provides standalone pages backed by the nearest applicable Knowledge Base. **Agents** appears for every role when Custom Agents is enabled; **Skills** appears only for editors and admins. These pages show only the selected component, while opening the Knowledge Base file still shows the complete tabbed editor. Agents defined by parent Knowledge Bases are inherited by child folders.

- **Viewers** see only enabled agents and can use Play to start an Explore conversation with one.
